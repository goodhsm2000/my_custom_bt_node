...
{ 
  ...
  int duration;
  if (!getInput<int>("wait_duration", duration)) {

    throw BT::RuntimeError("missing required input [wait_duration]");
  }

  pub_ = node_.advertise<geometry_msgs::Twist>("/cmd_vel", 1);

  uint32_t time = duration * 1000000;
